...
public:
   ...
   * annotated.
   * ------------------------------------------------------------ */
  bool should_show_overload(Node *n) {
  ...
  void append_overload_declaration(File *dest, Node *n, const String *name, bool in_class, bool is_static) {
    ...
    // FIXME: Add '/' here if there are parameters to indicate that the previous arguments are positional.
    // Otherwise, they could be key-value arguments, which isn't the case in the real function.
    // This requires Pyhton 3.7+
    const char *close = ")";
    Printv(dest, "def ", name, "(", parms, close, returnTypeAnnotation(n, true), ":\n", tab, tab4, "...\n", NIL);
  }
  ...
  */
  ...
  virtual int functionWrapper(Node *n) {
    ...
    if (!Getattr(n, "sym:overloaded")) {
      ...
      if (!builtin && shadow && !(shadow & PYSHADOW_MEMBER) && use_static_method && should_show_overload(n)) {
  ...
  virtual int memberfunctionHandler(Node *n) {
    ...
    if (should_show_overload(n) && shadow && !builtin && (!fastproxy || olddefs) && !Getattr(n, "feature:shadow")) {
  ...
  virtual int staticmemberfunctionHandler(Node *n) {
    ...
    if (should_show_overload(n) && shadow && !builtin && (!fastproxy || olddefs)) {
  ...
  virtual int constructorHandler(Node *n) {
    ...
    if (shadow) {
      ...
      if (should_show_overload(n) && !Getattr(n, "feature:shadow") && !builtin && !have_constructor && handled_as_init) {
